...
const InnerBoundary = createErrorBoundary<InnerProps>(props => ({
  onCatch(error, info) {
    console.error(`[Plugin:${props.pluginId}] Component error:`, error, info);
  },
  renderFallback(error, reset) {
    ...
    const handleRetry = () => {
      if (!canRetry) return;
      (props.retryCountRef as React.MutableRefObject<number>).current =
        (props.retryCountRef.current ?? 0) + 1;
      reset();
    };
