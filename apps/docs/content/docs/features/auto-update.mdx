...
<Callout>
  Switching from beta back to stable may offer a "downgrade" to the latest stable version if the
  current beta is newer.
</Callout>
...
<Callout type="warn">
  macOS requires apps to be code-signed for auto-install to work. Until Omniscribe is signed with an
  Apple Developer certificate, macOS users need to install updates manually.
</Callout>
