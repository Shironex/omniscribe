...
<Callout title="Tip">
  Not all AI assistants support MCP tools. Currently, Claude Code has built-in MCP support and will
  use these tools automatically when connected to Omniscribe.
</Callout>
...
<Callout>
  The MCP server runs locally on your machine. No data is sent to external services -- all
  communication happens between the AI assistant's terminal process and the Omniscribe app.
</Callout>

## Going Further — AI Capabilities

The Omniscribe Status server described above is only the built-in default. Omniscribe also ships a curated **AI Capabilities** registry so sessions can drive web apps and your own Electron app via Playwright. See [AI Capabilities](./ai-capabilities) for the full list and setup instructions (including the Electron CDP walkthrough).
