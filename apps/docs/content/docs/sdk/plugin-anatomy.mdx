...
description: Every file in a provider plugin, explained.
---
...
<Callout>
  The `as any` casts are required because `@omniscribe/plugin-api` is React-free (pure TypeScript),
  but the registration methods accept generic component types that are rendered by the React
  frontend.
</Callout>
...
See [Creating a Provider Plugin](/docs/sdk/creating-a-provider) for the complete wiring guide.
