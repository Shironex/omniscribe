...
describe('ClaudeUsageParserService', () => {
  let parser: ClaudeUsageParserService;

  beforeEach(() => {
    jest.useFakeTimers({ advanceTimers: true });
    parser = new ClaudeUsageParserService();
  });
  ...
  // ================================================================
  // stripAnsiCodes (shared utility used by parser)
  // ================================================================
  describe('stripAnsiCodes (shared utility)', () => {
  ...
  // toProviderUsageData
  // ================================================================
  describe('toProviderUsageData', () => {
    it('should map ClaudeUsage to ProviderUsageData with session and weekly metrics', () => {
      const usage = parser.parseUsageOutput(
        [
          'Current session',
          '  25% used',
      ...
      const providerData = parser.toProviderUsageData(usage);

      expect(providerData.metrics).toHaveLength(2);
      expect(providerData.metrics[0].name).toBe('Current Session');
      expect(providerData.metrics[0].percentage).toBe(25);
      expect(providerData.metrics[0].percentageType).toBe('used');
      expect(providerData.metrics[0].category).toBe('session');
      expect(providerData.metrics[1].name).toBe('Weekly (All Models)');
      expect(providerData.metrics[1].percentage).toBe(40);
      expect(providerData.metrics[1].category).toBe('weekly');
      expect(providerData.lastUpdated).toBeDefined();
      expect(providerData.userTimezone).toBeDefined();
    });

    it('should include model-specific metric when sonnet data is present', () => {
      const usage = parser.parseUsageOutput(
        [
          'Current session',
          '  10% used',
      ...
      expect(providerData.metrics).toHaveLength(3);
      expect(providerData.metrics[2].name).toBe('Weekly (Model)');
      expect(providerData.metrics[2].percentage).toBe(30);
      expect(providerData.metrics[2].category).toBe('weekly');
    });

    it('should NOT include model-specific metric when sonnet percentage is 0 and no text', () => {
      const usage = parser.parseUsageOutput('Current session\n  10% used\n');
  ...
  // ================================================================
  // parseUsageOutput() -- empty / malformed output
  // ================================================================
  describe('empty/malformed output', () => {
    ...
    it('should handle output with no recognizable sections', () => {
      const usage = parser.parseUsageOutput('Some random text\nThat has nothing useful\n');
