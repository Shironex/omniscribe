/**
 * Claude Usage Parser Service
 *
 * Parses the raw terminal output from `claude usage` command into structured
 * usage data. Handles session, weekly, and model-specific usage metrics with
 * support for various reset time formats.
 *
 * Extracted from apps/desktop/src/modules/usage/usage-output-parser.ts.
 ...
 * Claude Usage Parser Service.
 *
 * Parses raw CLI output from `claude usage` into structured usage data.
 * Supports multiple output formats including percentage values, duration-based
 * reset times, and date-based reset times.
 */
export class ClaudeUsageParserService {
  /**
   * Parse the Claude CLI usage output into ClaudeUsage format.
   *
   * This is the primary parsing method that returns the Claude-specific
   * usage format used by the existing frontend code.
   */
  ...
  /**
   * Convert ClaudeUsage to the plugin-api ProviderUsageData format.
   *
   * Maps the Claude-specific flat fields into the generic named metrics
   * array format used by the plugin API.
   */
  toProviderUsageData(usage: ClaudeUsage): ProviderUsageData {
    ...
    metrics.push({
      ...
      resetText: usage.sessionResetText || undefined,
      category: 'session',
    });
    ...
    metrics.push({
      ...
      resetText: usage.weeklyResetText || undefined,
      category: 'weekly',
    });

    if (usage.sonnetWeeklyPercentage > 0 || usage.sonnetResetText) {
      metrics.push({
        name: 'Weekly (Model)',
        percentage: usage.sonnetWeeklyPercentage,
        percentageType: 'used',
        resetText: usage.sonnetResetText || undefined,
        category: 'weekly',
      });
    }
